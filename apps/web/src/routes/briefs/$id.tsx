...
const briefJsonLd = ({
	...
	return {
		...
		// The line on the page carries the Article 50(4) disclosure for the reader;
		// this carries the same statement for whatever reads the page instead of
		// rendering it. schema.org has no field for "written by a model", so it goes
		// in the one place that takes an arbitrary claim without inventing a type.
		additionalProperty: {
			"@type": "PropertyValue",
			name: "aiGenerated",
			value: true,
	...
	return (
		<SiteShell>
			<article className={`brief-shell ${classes.page}`}>
				...
				{/*
				 * Article 50(4) of the AI Act: text generated by a model and published
				 * to inform the public has to say so. Above the player and above the
				 * script because both are generated, and on the page rather than in the
				 * terms because the obligation is to be read at first exposure.
				 */}
				<p className={classes.aiDisclosure}>{t.briefs.detail.aiDisclosure}</p>
